...
private:
    ...
    std::string replacePlaceholder(std::string input, const std::string &key, const std::string &value) noexcept;

    QFont m_timer_font, m_state_font, m_remaining_font, m_paused_font;
    bool m_timer_is_active{false}, m_hide_hour, m_show_notif, m_has_audio{false}, m_confirm_on_exit, m_state_shown,
        m_remaining_shown, m_paused_shown;

    int m_totalSeconds, m_pomodoro_count = 0, m_pomodoros_before_long_break = 4;
